...
export function mergeRecords<T extends GachaRecord>({
  ...
  if (previousRecentIndexInNewRecords === -1) {
    const errorData = {
      ...
      lastOldRecordTimestamp: sortedOldRecords[sortedOldRecords.length - 1].time,
    }
    if (dayjs.unix(sortedNewRecords[0].time).isBefore(dayjs.unix(sortedOldRecords[0].time))) {
    ...
    if (
      dayjs.unix(sortedNewRecords[0].time).isAfter(dayjs.unix(sortedOldRecords[0].time)) &&
      dayjs.unix(sortedNewRecords[0].time).isBefore(dayjs.unix(sortedOldRecords[sortedOldRecords.length - 1].time))
    ) {
      const error = new Error(
    ...
    const sortedNewRecordsWithoutOldRecords = sortedNewRecords.slice(previousRecentIndexInNewRecords + 1)
    const recordsToReturn = [...sortedOldRecords, ...sortedNewRecordsWithoutOldRecords]
    checkIfSorted(recordsToReturn)
    return recordsToReturn
  }
}
